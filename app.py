...
# Ensure 'model1.pkl' is in the same directory as this script
...
if st.button("Predict Exam Score"):
    ...
    # FIX: Safely extract the scalar value from the numpy array
    if hasattr(prediction_result, 'item'):
        prediction_value = prediction_result.item()
    else:
        prediction_value = prediction_result

    # Clamp the result between 0 and 100
    final_prediction = max(0.0, min(100.0, prediction_value))
    
    st.success(f"Predicted exam score: {final_prediction:.2f}")
